...
const ReviewsWithNonEmptyImages = ({ serviceId }) => {
  ...
  useEffect(() => {
    const getReviews = async () => {
      try {
        const response = await axios.post('http://3.26.59.191:5001/rating/getRatingsByService', { serviceId });
        setReviewsData(response.data.data);
        const response2 = await axios.post('http://3.26.59.191:5001/post/getPostsById', { serviceId });
