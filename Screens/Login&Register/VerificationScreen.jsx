...
export default function VerificationScreen({ navigation, route, props }) {
    ...
    const confirmCode = async () => {
        ...
        try {
            ...
            await axios.post(`http://192.168.254.111:5001/user/signup`, userData).then(async (res) => {
                if (res.status === 200) {
                    console.log("User created successfully");
                    await saveDetails(res.data.data._id);
                }
            })
            await axios.post("http://192.168.254.111:5001/user/login", {email: email, password: storeData.data.password}).then(async (res) => {
            ...
            if (res.data.status === 'SUCCESS') {
                ...
                await axios.post("http://192.168.254.111:5001/email_verification_otp/sendConfirmationEmail", {email: email, name: `${storeData.data.name.firstName} ${storeData.data.name.lastName}`, role: storeData.data.role});
    ...
    const fetchTempData = async () => {
        try {
            await axios.post(`http://192.168.254.111:5001/user/getTempDetails`, {email : email}).then((res) => {
    ...
    const verifyChangedNumber = async () => {
        try {
            await axios.post(`http://192.168.254.111:5001/user/getUserDetailsByMobile`, {mobile : mobile}).then(async (res) => {
                if (res.status === 200) {
                    setModalVisible(false);
            ...
            await axios.patch(`http://192.168.254.111:5001/user/updateTempNumber`, {email : email, mobile : mobile}).then((res) => {
                    if (res.status === 200) {
